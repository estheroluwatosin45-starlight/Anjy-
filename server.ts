...
async function startServer() {
  ...
  app.post('/api/generate-verse', async (req, res) => {
    try {
      ...
      // Basic cleanup in case it returned markdown block
  ...
  app.post('/api/backup', async (req, res) => {
    try {
      ...
      console.log(`[Backup System] Fetching records from database...`);
      ...
      console.log(`[Backup System] Generating document and emailing to ${email}...`);
      
      const { data, error } = await resend.emails.send({
        from: 'ANJY Backup <onboarding@resend.dev>', // Required for testing
        to: email,
      ...
      console.log(`[Backup System] Backup successfully sent!`);
